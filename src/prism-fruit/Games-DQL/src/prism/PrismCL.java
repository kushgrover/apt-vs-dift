	...
	private void parseArguments(String[] args) throws PrismException
	{
		...
		for (i = 0; i < args.length; i++) {
			...
			if (args[i].length() > 0 && args[i].charAt(0) == '-') {
				...
				//Parameters for grey or black DQL, namely overapproximation (OA) for states and available actions per state; also epsilon and delta
				//Format: S:xxx;Av:xxx;e:xxx;d:xxx;p:xxx
				else if (sw.equals("colourParams")) {
					if (i < args.length - 1) {
						if(!prism.setColourParams((String)args[++i])){
							errorAndExit("Problem when parsing colourParams. Format should be S:xxx;Av:xxx;e:xxx;d:xxx;p:xxx;post:xxx (where xxx is an int/double).");
						}
					} else {
						errorAndExit("No colourParams specified");
					}
				}
