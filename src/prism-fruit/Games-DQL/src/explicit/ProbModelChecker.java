...
public class ProbModelChecker extends NonProbModelChecker
{
	...
	protected MDPSolnMethod mdpSolnMethod = MDPSolnMethod.VALUE_ITERATION;
	...
	protected double termCritParam = 1e-6;
	...
	public enum MDPSolnMethod {
		VALUE_ITERATION, GAUSS_SEIDEL, POLICY_ITERATION, MODIFIED_POLICY_ITERATION, LINEAR_PROGRAMMING,BVI_A,BVI_C;
		public String fullName()
		{
			switch (this) {
	...
	public ProbModelChecker(PrismComponent parent) throws PrismException
	{
		...
		if (settings != null) {
			...
			} else if (s.equals("BVI_A")) {
				setSolnMethod(SolnMethod.BVI_A);
			} else if (s.equals("BVI_C")) {
				setSolnMethod(SolnMethod.BVI_C);
			} else if (s.equals("Gauss-Seidel")) {
	...
	private StateValues checkExpressionPATL(Model model, ExpressionPATL expr) throws PrismException
	{
		...
		if (relOp.isUpperBound() || relOp.isMin()) {
			// min
			min = true;
		} else if (relOp.isLowerBound()|| relOp.isMax()) {
			// max
			min = false;
		} else if (relOp.toString().equals("=")) {
			exact = true;
		} else {
		...
		if (type == ExpressionPATL.PRB) {
			...
			if (!exact) {
				...
				minmax.setMinMin(min,!min);
				Coalition coal = new Coalition();
				coal.setPlayers(new ArrayList(coalition));
				minmax.setCoalition(coal);
				BitSet statesOfInterest=new BitSet(model.getNumStates());
				statesOfInterest.set(0,model.getNumStates());
				probs = ((SMGModelChecker) this).checkProbPathFormula(model, expr.getExpressionProb().getExpression(), minmax,statesOfInterest);
				...
				Coalition coal = new Coalition();
				coal.setPlayers(new ArrayList(coalition));
				return ((SMGModelChecker) this).checkExactProbabilityFormula((SMG) model, expr.getExpressionProb(), coal,p,statesOfInterest);
			}
		} else if (type == ExpressionPATL.REW) {
			throw new PrismException("Can't handle PATL with rewards.");
