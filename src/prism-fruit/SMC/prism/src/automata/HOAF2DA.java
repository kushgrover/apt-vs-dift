...
public class HOAF2DA implements HOAConsumer {
	...
	private DA<NatBitSet, ? extends AcceptanceOmega> da;
	...
	private List<NatBitSet> acceptanceSets = null;
	/** The set of acceptance set indizes where state membership has to be inverted */
	private IntSet negateAcceptanceSetMembership = null;
	...
	public void addStartStates(List<Integer> stateConjunction)
		...
		startState = stateConjunction.get(0);
		knowStartState = true;
	}
	...
	public void notifyBodyStart() throws HOAConsumerException {
		...
		da = new DA<NatBitSet,AcceptanceGeneric>(size);
	...
	private AcceptanceGeneric prepareAcceptanceGeneric(BooleanExpression<AtomAcceptance> expr) throws HOAConsumerException
	{
		...
		case EXP_ATOM: {
			...
			if (acceptanceSets.get(index) == null) {
				// this acceptance set index has not been seen yet, create NatBitSet
				acceptanceSets.set(index, NatBitSets.set());
			}
			NatBitSet acceptanceSet = acceptanceSets.get(index);
	...
	private AcceptanceRabin prepareAcceptanceRabin() throws HOAConsumerException
	{
		...
		for (int i = 0; i< numberOfPairs; i++) {
			NatBitSet L = NatBitSets.set();
			NatBitSet K = NatBitSets.set();
	...
	private AcceptanceStreett prepareAcceptanceStreett() throws HOAConsumerException
	{
		...
		acceptanceSets = new ArrayList<>(numberOfPairs * 2);
		for (int i = 0; i< numberOfPairs; i++) {
			NatBitSet R = NatBitSets.set();
			NatBitSet G = NatBitSets.set();
	...
	private AcceptanceGenRabin prepareAcceptanceGenRabin() throws HOAConsumerException
	{
		...
		for (int i = 0; i< numberOfPairs; i++) {
			...
			ArrayList<NatBitSet> K_list = new ArrayList<>();
			for (int j = 0; j < numberOfKs[i]; j++) {
				NatBitSet K_j = NatBitSets.set();
				K_list.add(K_j);
	...
	public void addEdgeWithLabel(int stateId, BooleanExpression<AtomLabel> labelExpr, List<Integer> conjSuccessors, List<Integer> accSignature)
		...
		for (APMonom monom : labelExpressionToAPMonom(labelExpr)) {
			...
			while(it.hasNext()) {
				...
				int previousTo = da.getEdgeDestByLabel(stateId, NatBitSets.asSet(el));
				if (previousTo == to) {
				...
				da.addEdge(stateId, NatBitSets.asSet(el), to);
			}
		}
	}
	...
	public void notifyEnd() throws HOAConsumerException {
		...
		if (negateAcceptanceSetMembership != null) {
			negateAcceptanceSetMembership.forEach((IntConsumer) index -> acceptanceSets.get(index).flip(0, size));
		}
		clear();
	}
	...
	public DA<NatBitSet,? extends AcceptanceOmega> getDA() {
		return da;
	}
	...
	public static void main(String args[])
	{
		...
		try {
			...
			DA<NatBitSet, ? extends AcceptanceOmega> result;
			try {
