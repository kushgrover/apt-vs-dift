...
public class LTL2DA extends PrismComponent
{
	...
	public DA<NatBitSet, AcceptanceRabin> convertLTLFormulaToDRA(Expression ltl, Values constantValues) throws PrismException
	{
		return (DA<NatBitSet, AcceptanceRabin>) convertLTLFormulaToDA(ltl, constantValues, AcceptanceType.RABIN);
	}
	 ...
	 */
	public DA<NatBitSet, ? extends AcceptanceOmega> convertLTLFormulaToDA(Expression ltl, Values constants, AcceptanceType... allowedAcceptance)
			throws PrismException
	{
		DA<NatBitSet, ? extends AcceptanceOmega> result = null;
		...
		if (mode == Mode.RABINIZER) {
			List<AcceptanceType> acceptanceTypes = Arrays.asList(allowedAcceptance);
			if (!acceptanceTypes.contains(AcceptanceType.GENERALIZED_RABIN) && !acceptanceTypes.contains(AcceptanceType.RABIN)) {
				mode = Mode.LTL2DA;
			}
		}
		...
		if (result == null) {
			if (!containsTemporalBounds) {
				...
				} else if (mode == Mode.LTL2DA){
					// use jltl2dstar LTL2DA
					result = LTL2Rabin.ltl2da(ltl.convertForJltl2ba(), allowedAcceptance);
				} else {
					result = new LTL2Rabinizer(this).rabinize(ltl, allowedAcceptance);
				}
			} else {
	...
	public DA<NatBitSet, ? extends AcceptanceOmega> convertLTLFormulaToDAWithExternalTool(Expression ltl, Values constants, AcceptanceType... allowedAcceptance)
			throws PrismException
	{
		String ltl2daTool = getSettings().getString(PrismSettings.PRISM_LTL2DA_EXTERNAL_TOOL);
		...
		try {
			String syntax = getSettings().getString(PrismSettings.PRISM_LTL2DA_EXTERNAL_SYNTAX);
	...
	private enum Mode {
		LTL2DA, RABINIZER, EXTERNAL
	}

	/** Check whether we should use an external LTL->DA tool */
	private Mode getMode()
	{
		String modeString = getSettings().getString(PrismSettings.PRISM_LTL2DA_MODE);
		if ("External".equals(modeString)) {
			return Mode.EXTERNAL;
		}
		if ("LTL2DA".equals(modeString)) {
			return Mode.LTL2DA;
		}
		return Mode.RABINIZER;
	}
	...
	public static void main(String args[])
	{
		try {
			...
			DA<NatBitSet, ? extends AcceptanceOmega> da = ltl2da.convertLTLFormulaToDA(expr, null, AcceptanceType.RABIN, AcceptanceType.REACH);
