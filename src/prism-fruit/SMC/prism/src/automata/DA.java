...
public class DA<Symbol, Acceptance extends AcceptanceOmega>
{
	...
	/** The acceptance condition (as NatBitSets) */
	 ...
	 */
	public static void printLtl2dstar(DA<NatBitSet, AcceptanceRabin> dra, PrintStream out) throws PrismException {
		...
		for (int i_state = 0; i_state < dra.size(); i_state++) {
			...
			for (int pair = 0; pair < acceptance.size(); pair++) {
				if (acceptance.get(pair).getL().contains(i_state)) {
					out.print(" -"+pair);
				} else if (acceptance.get(pair).getK().contains(i_state)) {
					out.print(" +"+pair);
				}
			}
			out.println();
			...
			while (it.hasNext()) {
				...
				out.println(dra.getEdgeDestByLabel(i_state, NatBitSets.asSet(edge)));
			}
		}
	}
	...
	public void printHOA(PrintStream out) throws PrismException {
		...
		for (int i = 0; i < size(); i++) {
			...
			for (Edge edge : edges.get(i)) {
				...
				if (!(label instanceof NatBitSet))
					throw new PrismNotSupportedException("Can not print automaton with "+label.getClass()+" labels");
				String labelString = "["+APElement.toStringHOA((NatBitSet)label, apList.size())+"]";
	 ...
	 * i.e., a DA&lt;NatBitSet, AcceptanceRabin&gt; may become a DA&lt;NatBitSet, AcceptanceStreett&gt;
