...
public class MultiObjModelChecker extends PrismComponent
{
	...
	protected NondetModel constructDRAandProductMulti(NondetModel model, LTLModelChecker mcLtl, ModelChecker modelChecker, Expression ltl, int i,
			DA<NatBitSet, AcceptanceRabin> dra[], Operator operator, Expression pathFormula, JDDVars draDDRowVars, JDDVars draDDColVars, JDDNode ddStateIndex)
			throws PrismException
	{
		...
		List<JDDNode> labelDDs = new ArrayList<>();
	...
	protected JDDNode computeAcceptingEndComponent(DA<NatBitSet, AcceptanceRabin> dra, NondetModel modelProduct, JDDVars draDDRowVars, JDDVars draDDColVars,
	...
	protected void removeNonZeroMecsForMax(NondetModel modelProduct, LTLModelChecker mcLtl, List<JDDNode> rewardsIndex, OpsAndBoundsList opsAndBounds,
			int numTargets, DA<NatBitSet, AcceptanceRabin> dra[], JDDVars draDDRowVars[], JDDVars draDDColVars[]) throws PrismException
	{
		...
		if (!rmecs.equals(JDD.ZERO)) {
			...
			if (JDD.AreIntersecting(modelProduct.getStart(), rmecs)) {
				...
				IntList tmpmultitargetIDs = new IntArrayList();
				List<DA<NatBitSet, AcceptanceRabin>> tmpdra = new ArrayList<>();
				List<JDDVars> tmpdraDDRowVars = new ArrayList<>();
				List<JDDVars> tmpdraDDColVars = new ArrayList<>();
				int count = 0;
				for (int i = 0; i < numTargets; i++)
				...
				if (count > 0) {
					...
					DA<NatBitSet, AcceptanceRabin> newdra[] = new DA[count];
					...
					List<JDDNode> tmprewards = new ArrayList<>(1);
	...
	protected void checkConflictsInObjectives(NondetModel modelProduct, LTLModelChecker mcLtl, int conflictformulae, int numTargets,
			OpsAndBoundsList opsAndBounds, DA<NatBitSet, AcceptanceRabin> dra[], JDDVars draDDRowVars[], JDDVars draDDColVars[], List<JDDNode> targetDDs,
			List<ArrayList<JDDNode>> allstatesH, List<ArrayList<JDDNode>> allstatesL, List<JDDNode> multitargetDDs, IntList multitargetIDs)
			throws PrismException
	{
		DA<NatBitSet, AcceptanceRabin>[] tmpdra = new DA[conflictformulae];
		...
		List<IntList> tmpmultitargetIDs = new ArrayList<>();
	...
	protected void findTargetStates(NondetModel modelProduct, LTLModelChecker mcLtl, int numTargets, int conflictformulae, boolean reachExpr[],
			DA<NatBitSet, AcceptanceRabin> dra[], JDDVars draDDRowVars[], JDDVars draDDColVars[], List<JDDNode> targetDDs, List<JDDNode> multitargetDDs,
			IntList multitargetIDs) throws PrismException
	{
		int i, j;
		long l;

		// Compute all maximal end components
		List<ArrayList<JDDNode>> allstatesH = new ArrayList<>(numTargets);
		List<ArrayList<JDDNode>> allstatesL = new ArrayList<>(numTargets);
			...
			if (!reachExpr[i]) {
				...
				for (int k = 0; k < dra[i].getAcceptance().size(); k++) {
					...
					for (j = 0; j < dra[i].size(); j++) {
						if (!dra[i].getAcceptance().get(k).getL().contains(j)) {
							tmpH = JDD.SetVectorElement(tmpH, draDDRowVars[i], j, 1.0);
						}
						if (dra[i].getAcceptance().get(k).getK().contains(j)) {
		...
		if (conflictformulae > 1) {
			...
			// deal with reachability targets
			//multitargetIDs = new IntArrayList(tmpmultitargetIDs.size());
	...
	private int changeToInteger(IntList ids)
	{
		int k = 0;
	...
	protected Object computeMultiReachProbs(NondetModel model, LTLModelChecker mcLtl, List<JDDNode> transRewards, JDDNode start, List<JDDNode> targets,
			List<JDDNode> combinations, IntList combinationIDs, OpsAndBoundsList opsAndBounds, boolean hasconflictobjectives) throws PrismException
	{
		...
		for (i = 0; i < numTargets; i++) {
			...
			if (combinations != null) {
				for (j = 0; j < combinations.size(); j++) {
					if ((combinationIDs.getInt(j) & (1 << i)) > 0) {
			...
			List<JDDNode> rewards, OpsAndBoundsList opsAndBounds) throws PrismException
	{
		...
		List<Point> computedDirections = new ArrayList<>();
