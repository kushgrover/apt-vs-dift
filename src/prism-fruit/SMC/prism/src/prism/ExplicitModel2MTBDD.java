...
public class ExplicitModel2MTBDD
{
	...
	private void buildTrans(explicit.Model modelExpl) throws PrismException
	{
		...
		if (modelType == ModelType.DTMC || modelType == ModelType.CTMC) {
			...
			for (r = 0; r < numStates; r++) {
				Iterator<Int2DoubleMap.Entry> iter = dtmc.getTransitionsIterator(r);
				while (iter.hasNext()) {
		...
		// Case for MDPs...
			...
			for (r = 0; r < numStates; r++) {
				...
				for (k = 0; k < nc; k++) {
					for (Int2DoubleMap.Entry e : mdp.getTransitions(r, k)) {
						c = e.getIntKey();
						d = e.getDoubleValue();
						a = "";
						// construct element of matrix mtbdd
	...
	private void buildInit() throws PrismException
	{
		...
		IntIterator iterator = modelExpl.getInitialStates().iterator();
		while (iterator.hasNext()) {
			start = JDD.Or(start, encodeState(iterator.nextInt()));
		}
	}
