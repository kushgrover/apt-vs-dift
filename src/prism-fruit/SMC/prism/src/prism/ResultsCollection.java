...
public class ResultsCollection
{
	...
	public ResultsCollection(UndefinedConstants uCons, String resultName)
	{
		...
		rangingConstants = new ArrayList<DefinedConstant>();
		List<DefinedConstant> tmpRangingConstants = uCons.getRangingConstants();
	...
	{
		return resultListeners.remove(resultListener);
	}
	...
	public String[] getHeadingsArray()
	{
		...
		for (i = 0; i < rangingConstants.size(); i++) {
			res[i] = rangingConstants.get(i).getName();
		}
	...
	public String toStringPartial(Values partial, boolean pv, String sep, String eq, boolean header) throws PrismException
	{
		...
		noVars = true;
		for (i = 0; i < rangingConstants.size(); i++) {
			if (!partial.contains(rangingConstants.get(i).getName())) {
				noVars = false;
				break;
			}
		}
		...
		if (header) {
			first = true;
			for (i = 0; i < rangingConstants.size(); i++) {
				name = rangingConstants.get(i).getName();
