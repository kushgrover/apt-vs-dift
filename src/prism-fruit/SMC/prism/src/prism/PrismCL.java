...
import simulator.UnboundedSimulationMethod;
...
import simulator.method.MonitorMethod;
import simulator.method.SPRTMethod;
import simulator.method.SPRTMethodError;
import simulator.method.SSPMethod;
import simulator.method.SSPMethodError;
...
public class PrismCL implements PrismModelListener
{
	...
	private boolean simCandidate = false;
	private boolean simReach = false;
	private boolean simRandomTerm = false;
	private boolean simWhitebox = false;
	private double simMinProb;
	private double simFalseCnd;
	private double simRatio;
	private double simTermProb;
	private int simCheckBound;

	private boolean simMinProbGiven = false;
	private boolean simFalseCndGiven = false;
	private boolean simRatioGiven = false;
	private boolean simTermProbGiven = false;
	private boolean simCheckBoundGiven = false;
		...
		for (i = 0; i < undefinedMFConstants.getNumModelIterations(); i++) {
			...
			for (j = 0; j < numPropertiesToCheck; j++) {
				...
				if (simulate && undefinedConstants[j].getNumPropertyIterations() > 1) {
					try {
						Pair<SimulationMethod, UnboundedSimulationParameters> pair = processSimulationOptions(propertiesToCheck.get(j).getExpression());
						simMethod = pair.first;
						UnboundedSimulationParameters usp = pair.second;
		...
		for (i = 0; i < args.length; i++) {
			...
			if (!args[i].isEmpty() && args[i].charAt(0) == '-') {
				...
				// simulation-based model checking methods
					...
					if (i < args.length - 1) {
						s = args[++i];
						if (s.equals("ci") || s.equals("aci") || s.equals("apmc") || s.equals("sprt") || s.equals("ssp") || s.equals("sprte") || s.equals("sspe") || s.equals("monitor"))
							simMethodName = s;
						else
							errorAndExit("Unrecognised option for -" + sw + " switch (options are: ci, aci, apmc, sprt, sprte, ssp, sspe, monitor)");
					} else {
				...
				else if (sw.equals("simcandidate")){
					simCandidate = true;
				} else if (sw.equals("simreach")){
					simReach = true;
				} else if (sw.equals("simrandomterm")){
					simRandomTerm = true;
				} else if (sw.equals("simwhitebox")){
					simWhitebox = true;
				}
				else if (sw.equals("simfalsecnd")) {
					if (i < args.length - 1) {
						try {
							simFalseCnd = Double.parseDouble(args[++i]);
							if (simFalseCnd <= 0 || simFalseCnd > 1)
								throw new NumberFormatException("");
							simFalseCndGiven = true;
						} catch (NumberFormatException e) {
							errorAndExit("Invalid value for -" + sw + " switch");
						}
					} else {
				...
				else if (sw.equals("simminprob")) {
					if (i < args.length - 1) {
						try {
							simMinProb = Double.parseDouble(args[++i]);
							if (simMinProb < 0 || simMinProb > 1)
								throw new NumberFormatException("");
							simMinProbGiven = true;
						} catch (NumberFormatException e) {
							errorAndExit("Invalid value for -" + sw + " switch");
						}
					} else {
				...
				else if (sw.equals("simratio")) {
					if (i < args.length - 1) {
						try {
							simRatio = Double.parseDouble(args[++i]);
							if (simRatio <= 0 || simRatio > 1)
								throw new NumberFormatException("");
							simRatioGiven = true;
						} catch (NumberFormatException e) {
							errorAndExit("Invalid value for -" + sw + " switch");
				...
				else if (sw.equals("simtermprob")) {
					if (i < args.length - 1) {
						try {
							simTermProb = Double.parseDouble(args[++i]);
							if (simTermProb <= 0 || simTermProb > 1)
								throw new NumberFormatException("");
							simTermProbGiven = true;
						} catch (NumberFormatException e) {
							errorAndExit("Invalid value for -" + sw + " switch");
						}
					} else {
				...
				else if (sw.equals("simcheckbound")) {
					if (i < args.length - 1) {
						try {
							simCheckBound = Integer.parseInt(args[++i]);
							if (simCheckBound < 1)
								throw new NumberFormatException("");
							simCheckBoundGiven = true;
						} catch (NumberFormatException e) {
							errorAndExit("Invalid value for -" + sw + " switch");
						}
					} else {
	 ...
	 */
	private Pair<SimulationMethod,UnboundedSimulationParameters> processSimulationOptions(Expression expr) throws PrismException
	{
		...
		if (!simMaxPathGiven)
			simMaxPath = prism.getSettings().getLong(PrismSettings.SIMULATOR_DEFAULT_MAX_PATH);
		if (!simFalseCndGiven)
			simFalseCnd = prism.getSettings().getDouble(PrismSettings.SIMULATOR_DEFAULT_FALSECND);
		if (!simRatioGiven)
			simRatio = prism.getSettings().getDouble(PrismSettings.SIMULATOR_DEFAULT_RATIO);
		if (!simTermProbGiven)
			simTermProb = prism.getSettings().getDouble(PrismSettings.SIMULATOR_DEFAULT_TERMPROB);
		if (!simCheckBoundGiven)
			simCheckBound = prism.getSettings().getInteger(PrismSettings.SIMULATOR_DEFAULT_CHECKBOUND);
		...
		if ((simCandidate && simReach) || (simReach && simRandomTerm) || (simCandidate && simReach)){
			throw new PrismException("At most one the options -simcandidate, -simreach or -simrandomterm can be set");
		}

		UnboundedSimulationMethod usm = UnboundedSimulationMethod.UNTIL;
		if (simCandidate)
			usm = UnboundedSimulationMethod.CANDIDATE;
		else if (simReach)
			usm = UnboundedSimulationMethod.REACHABILITY;
		else if (simRandomTerm)
			usm = UnboundedSimulationMethod.RANDOMTERM;

		int engine = prism.getSettings().getChoice(PrismSettings.PRISM_ENGINE);
		UnboundedSimulationParameters usp = new UnboundedSimulationParameters(usm, simWhitebox, simFalseCnd, simMinProb, simRatio, simTermProb, simCheckBound, engine);

		if (simRandomTerm && !simMethodName.equals("ci")){
			throw new PrismException("Option -simmethod must be \"ci\", if -simrandomterm is set");
		}
		...
		// SPRT with error correction
		else if (simMethodName.equals("sprte")) {
			if (isQuant) {
				throw new PrismException("Cannot use SPRT on a quantitative (=?) property");
			}

			if (!simMinProbGiven){
				throw new PrismException("Option -simminprob required for the SPRTE method");
			}

			if (!simFalseCndGiven){
				throw new PrismException("SSPE method requires the option  -simfalsecnd");
			}

			aSimMethod = new SPRTMethodError(simConfidence, simConfidence, simWidth, simFalseCnd, simMinProb);
		...
		// SSP
		else if (simMethodName.equals("ssp")) {
			if (isQuant) {
				throw new PrismException("Cannot use SSP on a quantitative (=?) property");
			}

			aSimMethod = new SSPMethod(simConfidence, simConfidence, simWidth);
			if (simApproxGiven) {
				mainLog.printWarning("Option -simapprox is not used for the SSP method and is being ignored");
			}
			if (simNumSamplesGiven) {
				mainLog.printWarning("Option -simsamples is not used for the SSP method and is being ignored");
			}
		}
		// SSP with error correction
		else if (simMethodName.equals("sspe")) {
			if (isQuant) {
				throw new PrismException("Cannot use SSP on a quantitative (=?) property");
			}

			if (!simMinProbGiven) {
				throw new PrismException("Option -simminprob required for the SSPE method");
			}
			...
			aSimMethod = new SSPMethodError(simConfidence, simConfidence, simWidth, simFalseCnd, simMinProb);
		...
		else if (simMethodName.equals("monitor")) {
			mainLog.printWarning("You are using the monitor simulation method, which is only a quick hack and only stable for exactly the 8 experiments it was used for.");
			if (!simFalseCndGiven) {
				throw new PrismException("Monitor method requires the option  -simfalsecnd, which is used to determine variant."
						+ " If variant>0.5, we are bold, else cautious. This is applied in SamplerCandidateLTL when deciding visitNo (k_i).");
			}
			aSimMethod = new MonitorMethod(simFalseCnd);
		}
		else
			throw new PrismException("Unknown simulation method \"" + simMethodName + "\"");

		Pair<SimulationMethod,UnboundedSimulationParameters> pair = new Pair<>(aSimMethod, usp);
		return pair;
	}
	...
	private void printHelp()
	{
		...
		mainLog.println();
		mainLog.println("-simcandidate .................. Simulation for arbitrary LTL properties, based on on-the-fly identification of BSCCs (options -simfalsecnd, -simminprob, -simration apply)");
		mainLog.println("-simreach ...................... Simulation for unbounded until, based on white-box reachability analysis (option -explicit applies)");
		mainLog.println("-simrandomterm ................. Simulation for unbounded until with random termination (option -simtermprob applies)");
		mainLog.println("-simfalsecnd <x> ............... Upper bound on the probability that a SC is falsely identified as a BSCC");
		mainLog.println("-simminprob <x> ................ Lower bound on the minimum transition probability");
		mainLog.println("-simratio <x> .................. How much the error bound decreases, when a new candidate is found");
		mainLog.println("-simtermprob <x> ............... Probability of randomly terminating the current path");
