...
public class LTLModelChecker extends PrismComponent
{
	 ...
	 */
	public Expression checkMaximalStateFormulas(ModelChecker mc, Model model, Expression expr, List<JDDNode> labelDDs) throws PrismException
	{
		// A state formula
	 ...
	 * @return the DA
	 */
	public DA<NatBitSet, ? extends AcceptanceOmega> constructDAForLTLFormula(ModelChecker mc, Model model, Expression expr, List<JDDNode> labelDDs,
	 ...
	 */
	public ProbModel constructProductMC(DA<NatBitSet, ? extends AcceptanceOmega> da, ProbModel model, List<JDDNode> labelDDs) throws PrismException
	{
	 ...
	 */
	public ProbModel constructProductMC(DA<NatBitSet, ? extends AcceptanceOmega> da, ProbModel model, List<JDDNode> labelDDs, JDDVars daDDRowVarsCopy,
			JDDVars daDDColVarsCopy)
	 ...
	 * @param labelDDs BDDs giving the set of states for each AP in the DA
	 */
	public NondetModel constructProductMDP(DA<NatBitSet, ? extends AcceptanceOmega> da, NondetModel model, List<JDDNode> labelDDs) throws PrismException
	{
	 ...
	 * @param daDDColVarsCopy (Optionally) empty JDDVars object to obtain copy of DD col vars for DA
	 */
	public NondetModel constructProductMDP(DA<NatBitSet, ? extends AcceptanceOmega> da, NondetModel model, List<JDDNode> labelDDs, JDDVars daDDRowVarsCopy,
			JDDVars daDDColVarsCopy)
			throws PrismException
	{
			...
			boolean allInit, JDDNode init) throws PrismException
	{
		...
		modelProd.setSynchs(new ArrayList<>(model.getSynchs()));
	 ...
	 */
	public JDDNode buildTransMask(DA<NatBitSet, ? extends AcceptanceOmega> da, List<JDDNode> labelDDs, JDDVars allDDRowVars, JDDVars allDDColVars,
			JDDVars daDDRowVars,
			JDDVars daDDColVars)
	{
		...
		for (i = 0; i < numStates; i++) {
			...
			for (j = 0; j < numEdges; j++) {
				...
				for (k = 0; k < numAPs; k++) {
					...
					JDD.Ref(exprBDD);
					if (!da.getEdgeLabel(i, j).contains(k)) {
	 ...
	 */
	public JDDNode buildStartMask(DA<NatBitSet, ? extends AcceptanceOmega> da, List<JDDNode> labelDDs, JDDVars daDDRowVars)
	{
			...
			throws PrismException
	{
		...
		default:
			throw new PrismNotSupportedException(
					"Computing the accepting EC states for " + acceptance.getType().getName() + " acceptance is not yet implemented (symbolic engine)");
		}
	}
	...
	public JDDNode findMultiAcceptingStates(DA<NatBitSet, AcceptanceRabin> dra, NondetModel model, JDDVars draDDRowVars, JDDVars draDDColVars, boolean fairness,
	...
	public void findMultiConflictAcceptingStates(DA<NatBitSet, AcceptanceRabin>[] dra, NondetModel model, JDDVars[] draDDRowVars, JDDVars[] draDDColVars,
			List<JDDNode> targetDDs,
			List<List<JDDNode>> allstatesH, List<List<JDDNode>> allstatesL, List<JDDNode> combinations, List<IntList> combinationIDs)
			throws PrismException
	{
	...
	private void computeCombinations(DA<NatBitSet, AcceptanceRabin>[] dra, NondetModel model, JDDVars[] draDDRowVars, JDDVars[] draDDColVars,
			List<JDDNode> targetDDs,
			List<List<JDDNode>> allstatesH, List<List<JDDNode>> allstatesL, List<QueueElement> queue, int sp) throws PrismException
	{
		...
		for (int i = e.next; i < dra.length; i++) {
			...
			if (!newstatesH.isEmpty() /*&& i+1 < dra.length*/) {
				...
				IntList ids = new IntArrayList(e.draIDs);
				ids.add(i);
	...
	public List<JDDNode> findBottomEndComponents(NondetModel model, JDDNode states) throws PrismException
	{
		...
		List<JDDNode> becs = new ArrayList<>();
		JDDNode out;
...
class QueueElement
{
	...
	public QueueElement(List<JDDNode> statesH, List<JDDNode> statesL, JDDNode targetDD, IntList draIDs, int next)
	{
