...
/**
 * A Rabin acceptance condition (based on NatBitSet state sets).
       ...
       implements AcceptanceOmega
{
	...
	public static class RabinPair {
		 ...
		 *  (F G !"L")  &  (G F "K")
		 */
		public RabinPair(NatBitSet L, NatBitSet K) {
			this.L = L;
			this.K = K;
		}
		...
		{
			AcceptanceGeneric genericL = new AcceptanceGeneric(AcceptanceGeneric.ElementType.FIN, L.clone());
			AcceptanceGeneric genericK = new AcceptanceGeneric(AcceptanceGeneric.ElementType.INF, K.clone());
		...
		{
			if (L.contains(stateIndex)) {
				return "-"+pairIndex;
			} else if (K.contains(stateIndex)) {
				return "+"+pairIndex;
			} else {
				return "";
			}
		...
		{
			return new RabinPair(L.clone(), K.clone());
		}
	...
	public AcceptanceStreett complementToStreett()
	{
		...
		for (RabinPair accPairRabin : this) {
			NatBitSet R = accPairRabin.getK().clone();
			NatBitSet G = accPairRabin.getL().clone();
	...
	public String getSignatureForStateHOA(int stateIndex)
	{
		...
		for (int pairIndex=0; pairIndex<size(); pairIndex++) {
			...
			if (pair.getK().contains(stateIndex)) {
