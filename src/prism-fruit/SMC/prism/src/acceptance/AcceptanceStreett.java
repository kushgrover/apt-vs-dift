...
/**
 * A Streett acceptance condition (based on NatBitSet state sets).
       ...
       implements AcceptanceOmega
{
	...
	public static class StreettPair {
		 ...
		 * 	 (G F "R") -> (G F "G")
		 */
		public StreettPair(NatBitSet R, NatBitSet G)
		{
			this.R = R;
			this.G = G;
		}
		...
		/** Get the state set R */
		public NatBitSet getR()
		{
			return R;
		}

		/** Get the state set G */
		public NatBitSet getG()
		{
			return G;
		}
		...
		{
			AcceptanceGeneric genericR = new AcceptanceGeneric(AcceptanceGeneric.ElementType.FIN, R.clone());
			AcceptanceGeneric genericG = new AcceptanceGeneric(AcceptanceGeneric.ElementType.INF, G.clone());
			//      G F "R" -> G F "G"
			// <=>  ! G F "R"  | G F "G"
			// <=>  F G ! "R"  | G F "G"
			// <=>  Fin(R) | Inf(G)
		...
		{
			if (G.contains(stateIndex)) {
				return "+"+pairIndex;
			} else if (R.contains(stateIndex)) {
				return "-"+pairIndex;
			} else {
				return "";
			}
		...
		{
			return new StreettPair(R.clone(), G.clone());
		}
	...
	public AcceptanceRabin complementToRabin()
	{
		...
		for (StreettPair accPairStreett : this) {
			NatBitSet L = accPairStreett.getG().clone();
			NatBitSet K = accPairStreett.getR().clone();
	...
	public String getSignatureForStateHOA(int stateIndex)
	{
		...
		for (int pairIndex=0; pairIndex<size(); pairIndex++) {
			StreettPair pair = get(pairIndex);
			if (pair.getR().contains(stateIndex)) {
				result += (result.isEmpty() ? "" : " ") + pairIndex*2;
			}
			if (pair.getG().contains(stateIndex)) {
