...
/**
 * A Generalized Rabin acceptance condition (based on NatBitSet state sets)
       ...
       implements AcceptanceOmega
{
	...
	public static class GenRabinPair {
		...
		private ArrayList<NatBitSet> K_list;

		/** Constructor with L and K_j state sets */
		public GenRabinPair(NatBitSet L, ArrayList<NatBitSet> K_list) {
			this.L = L;
			this.K_list = K_list;
		}

		/** Get the state set L */
		public NatBitSet getL()
		{
			return L;
		}
		...
		{
			ArrayList<NatBitSet> newK_list = new ArrayList<NatBitSet>();
			for (NatBitSet K_j : K_list)
				newK_list.add((NatBitSet) K_j.clone());
			return new GenRabinPair((NatBitSet)L.clone(), newK_list);
		}

		public AcceptanceGeneric toAcceptanceGeneric()
		{
			AcceptanceGeneric genericL = new AcceptanceGeneric(AcceptanceGeneric.ElementType.FIN, (NatBitSet) L.clone());
			if (getNumK() == 0) {
				return genericL;
			}
			AcceptanceGeneric genericKs = null;
			for (NatBitSet K : K_list) {
				AcceptanceGeneric genericK = new AcceptanceGeneric(AcceptanceGeneric.ElementType.INF, (NatBitSet) K.clone());
				if (genericKs == null) {
					genericKs = genericK;
				} else {
	...
	public String getSignatureForStateHOA(int stateIndex)
	{
		...
		int set_index = 0;
		for (GenRabinPair pair : this) {
			if (pair.getL().contains(stateIndex)) {
				result += (result.isEmpty() ? "" : " ") + set_index;
			}
			set_index++;
			for (int i=0; i < pair.getNumK(); i++) {
				if (pair.getK(i).contains(stateIndex)) {
					result += (result.isEmpty() ? "" : " ") + set_index;
				}
				set_index++;
			}
