       ...
       implements AcceptanceOmegaDD
{
	...
	/**
	 * Constructor, from a NatBitSet-based AcceptanceGenRabin.
	 *
	 * @param acceptance the NatBitSet-based acceptance condition
	...
	public AcceptanceGenRabinDD(AcceptanceGenRabin acceptance, JDDVars ddRowVars)
	{
		for (AcceptanceGenRabin.GenRabinPair pair : acceptance) {
			...
			IntIterator lIterator = pair.getL().iterator();
			while (lIterator.hasNext()) {
				int i = lIterator.nextInt();
			...
			for (int j = 0; j < n; j++) {
				...
				IntIterator kjIterator = pair.getK(j).iterator();
				while (kjIterator.hasNext()) {
					int i = kjIterator.nextInt();
