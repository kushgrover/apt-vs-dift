...
public class DebugJDD
{
	 ...
	 */
	protected static Long2ObjectMap<DebugJDDNode> nodes = new Long2ObjectLinkedOpenHashMap<>();

	/** A map from DdNode pointers to the current sum of the nodeRefs of all JDDNodes for that pointer */
	protected static Long2IntMap javaRefs = new Long2IntOpenHashMap();

	/** An optional set of DebugJDDNode IDs that will be traced */
	protected static LongSet traceIDs = null;
	...
	public static void endLifeCycle()
	{
		...
		Long2IntMap externalRefCounts = getExternalRefCounts();
		if (externalRefCounts.isEmpty()) {
			// everthing is fine
			return;
		}
		...
		System.out.flush();
		for (Long2IntMap.Entry extRef : externalRefCounts.long2IntEntrySet()) {
			long ptr = extRef.getLongKey();
			...
			System.out.println("DdNode ptr=0x" + Long.toHexString(ptr)
			                   + ", " + nodeInfo(ptr) + " has " + extRef.getIntValue() + " remaining external references.");
			if (!posRefNodes.isEmpty()) {
	...
	public static Long2IntMap getExternalRefCounts()
	{
		Long2IntMap result = new Long2IntAVLTreeMap();
