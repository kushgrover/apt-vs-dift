...
final class MutablePMC {
	...
	MutablePMC(FunctionFactory functionFactory, int numStates, boolean useRewards, boolean useTime) {
		...
		for (int state = 0; state < numStates; state++) {
			transitionTargets.add(new IntArrayList());
			transitionProbs.add(new LinkedList<>());
			incoming.add(new IntArrayList());
		}
		this.useRewards = useRewards;
		this.useTime = useTime;
		initStates = NatBitSets.boundedSet(numStates);
		targetStates = NatBitSets.boundedSet(numStates);
		if (useRewards) {
	...
	Function getSelfLoopProb(int state)
	{
		...
		IntListIterator toIter = transitionTargets.get(state).listIterator();
		ListIterator<Function> valIter = transitionProbs.get(state).listIterator();
		while (toIter.hasNext()) {
			int to = toIter.nextInt();
	...
	{
		IntList loop = new IntArrayList();
		loop.add(state);
		transitionTargets.set(state, loop);
		transitionProbs.set(state, Collections.singletonList(functionFactory.getOne()));
	}
	...
	{
		return initStates.contains(state);
	}
