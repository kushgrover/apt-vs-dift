...
public final class ParamModel extends ModelExplicit
{
	 ...
	 */
	public Iterator<Int2ObjectMap.Entry<Function>> getTransitionsIterator(final int s, final int i)
	{
		return new Iterator<Int2ObjectMap.Entry<Function>>()
		{
			private final int start = choiceBegin(stateBegin(s) + i);
			private int col = start;
			private final int end = choiceBegin(stateBegin(s) + i + 1);
			...
			@Override
			public Int2ObjectMap.Entry<Function> next()
			{
				if (!(col < end)) {
				...
				int i = col;
				col++;
				return new AbstractInt2ObjectMap.BasicEntry<>(cols[i], nonZeros[i]);
			}
		};
	}
	...
	protected void exportTransitionsToDotFile(int i, PrismLog out)
	{
		...
		for (int j = 0; j < numChoices; j++) {
			...
			Iterator<Int2ObjectMap.Entry<Function>> it = getTransitionsIterator(i, j);
			while (it.hasNext()) {
				Int2ObjectMap.Entry<Function> e = it.next();

				// Note: For CTMCs, param stores the embedded DTMC, so we output that

				if (!modelType.nondeterministic()) {
					out.print(i + " -> " + e.getIntKey() + " ");
				} else {
					out.print(nij + " -> " + e.getIntKey() + " ");
				}
	...
	ParamModel instantiate(Point point)
	{
		...
		result.initialStates = new IntArrayList(this.initialStates);
