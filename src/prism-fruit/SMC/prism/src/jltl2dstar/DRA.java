...
public class DRA extends DA {
	 ...
	 */
	public automata.DA<NatBitSet,? extends AcceptanceOmega> createPrismDA() throws PrismException
	{
		int numStates = size();
		if (!isStreett()) {
			// Rabin
			automata.DA<NatBitSet, AcceptanceRabin> draNew;
			...
			// Streett
			automata.DA<NatBitSet, AcceptanceStreett> dsaNew;
	 ...
	 */
	private void createPrismDA(automata.DA<NatBitSet, ?> da) throws PrismException
	{
	...
	private AcceptanceRabin createRabinAcceptance() throws PrismException {
		...
		for (int i = 0; i < acc.size(); i++) {
			...
			NatBitSet newL = NatBitSets.asSet(acc.getAcceptance_U(i)).clone();
			NatBitSet newK = NatBitSets.asSet(acc.getAcceptance_L(i)).clone();
	...
	private AcceptanceStreett createStreettAcceptance() throws PrismException {
		...
		for (int i = 0; i < acc.size(); i++) {
			...
			NatBitSet newR = NatBitSets.asSet(acc.getAcceptance_L(i)).clone();
			NatBitSet newG = NatBitSets.asSet(acc.getAcceptance_U(i)).clone();
