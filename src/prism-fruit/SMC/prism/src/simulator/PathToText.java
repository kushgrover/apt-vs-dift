...
public class PathToText extends PathDisplayer
{
	...
	private void displayState(State state)
	{
		...
		if (varsToShow == null) {
			...
			varsToShow.forEach((IntConsumer) v -> {
				log.print(getColSep());
	...
	{
		return IntIterators.any(FastUtils.iterator(varsToShow, numVars), v -> !Objects.equals(newState.varValues[v], lastState.varValues[v]));
	}
