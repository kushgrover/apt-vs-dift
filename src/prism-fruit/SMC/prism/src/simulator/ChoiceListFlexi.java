...
public class ChoiceListFlexi implements Choice
{
	...
	public ChoiceListFlexi()
	{
		...
		probability = new DoubleArrayList();
	}
	...
	public ChoiceListFlexi(ChoiceListFlexi ch)
	{
		...
		for (List<Update> list : ch.updates) {
			List<Update> listNew = new ArrayList<>(list);
			updates.add(listNew);
		}
		probability = new DoubleArrayList(ch.probability);
	}
	...
	public void productWith(ChoiceListFlexi ch)
	{
		...
		for (i = 1; i < n; i++) {
			...
			for (j = 0; j < n2; j++) {
				...
				list.addAll(updates.get(j));
				list.addAll(ch.updates.get(i));
		...
		for (j = 0; j < n2; j++) {
			...
			probability.set(j, pi * probability.getDouble(j));
		}
	}
		...
		for (Update up : updates.get(i)) {
			...
			for (j = 0; j < n; j++) {
				if (first)
					...
					s.append(", ");
				s.append(up.getVar(j)).append("'=").append(up.getExpression(j).evaluate(currentState));
			}
		}
		return s.toString();
	}
