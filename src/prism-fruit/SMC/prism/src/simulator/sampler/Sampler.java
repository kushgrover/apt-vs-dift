...
public abstract class Sampler
{
	...
	// which method to use for unbounded properties?
	private boolean simCandidate;
	private boolean simReach;
	private boolean sinRandomTerm;
	 ...
	 */
	public static Sampler createSampler(Expression expr, ModulesFile mf, Values constantValues, UnboundedSimulationParameters usp) throws PrismException
	{
		...
		if (expr instanceof ExpressionProb) {
			...
			if (propProb.getExpression().isSimplePathFormula()) {
				sampler = createSamplerForProbPathPropertySimple(propProb.getExpression(), mf, constantValues, usp);
			} else {
				switch (usp.getUnboudedSimulationMethod()) {
				case CANDIDATE:
					sampler = new SamplerCandidateLTL(mf, expr, constantValues, usp);
					break;
				default:
					throw new PrismException("LTL-style path formulas are not supported by the simulation method");
				}
		...
		// R=?
			...
			sampler = createSamplerForRewardProperty((ExpressionReward) expr, mf, constantValues, usp);
		}
		// Neither
		else {
	...
	private static SamplerBoolean createSamplerForProbPathPropertySimple(Expression expr, ModulesFile mf, Values constantValues,
			UnboundedSimulationParameters usp) throws PrismException
	{
		...
		if (expr instanceof ExpressionUnaryOp) {
			...
			if (exprUnary.getOperator() == ExpressionUnaryOp.PARENTH) {
				// Recurse
				return createSamplerForProbPathPropertySimple(exprUnary.getOperand(), mf, constantValues, usp);
			}
			// Negation
			else if (exprUnary.getOperator() == ExpressionUnaryOp.NOT) {
				// Recurse, then negate meaning
				SamplerBoolean sampler = createSamplerForProbPathPropertySimple(exprUnary.getOperand(), mf, constantValues, usp);
				sampler.negate();
				return sampler;
			}
		}
		// Temporal operators
			...
			// Until
				...
				if (exprTemp.hasBounds()) {
					...
					switch (usp.getUnboudedSimulationMethod()) {
					case CANDIDATE:
						return new SamplerCandidateUntil(mf, exprTemp, constantValues, usp);
					case RANDOMTERM:
						return new SamplerRandomTermination(exprTemp, usp.getTerminationProb());
					case REACHABILITY:
						// TODO provide a prism object
						return new SamplerUntilWithReachability(mf, exprTemp, usp.getEngine());
					case UNTIL:
						return new SamplerUntil(exprTemp);
					}
				}
			...
			// Anything else - convert to until and recurse
				...
				return createSamplerForProbPathPropertySimple(exprTemp.convertToUntilForm(), mf, constantValues, usp);
			}
		}
	...
	private static SamplerDouble createSamplerForRewardProperty(ExpressionReward expr, ModulesFile mf, Values constantValues, UnboundedSimulationParameters usp)
			throws PrismException
	{
		...
		case ExpressionTemporal.R_S:
			if (mf.getModelType().continuousTime()) {
				throw new PrismException("Can't create sampler for property \"" + expr + "\" for a continous model");
			} else {
				// Discrete-time mean payoff
				return new SamplerCandidateMeanPayoff(mf, constantValues, rsi, expr, usp);
			}
