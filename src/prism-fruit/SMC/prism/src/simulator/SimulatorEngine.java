...
public class SimulatorEngine extends PrismComponent
{
	 ...
	 * Any constants/formulas etc. appearing in the label must have been defined in the current model.
	 * If there are additional constants (e.g. from a properties file),
	 * then use the {@link #addProperty(Expression, PropertiesFile, UnboundedSimulationParameters)} method.
	 */
	public int addProperty(Expression prop, UnboundedSimulationParameters usp) throws PrismException
	{
		return addProperty(prop, null, usp);
	}
	 ...
	 */
	public int addProperty(Expression prop, PropertiesFile pf, UnboundedSimulationParameters usp) throws PrismException
	{
		// Take a copy
		...
		// Create sampler
		Sampler sampler = Sampler.createSampler(propNew, modulesFile, mfConstants, usp);
	 ...
	 */
	public void exportPath(File file, boolean timeCumul, String colSep, IntArrayList vars) throws PrismException
	{
		PrismLog log;
		if (path == null)
			...
			SimulationMethod simMethod, UnboundedSimulationParameters usp) throws PrismException
	{
		...
		exprs.add(expr);
		res = modelCheckMultipleProperties(modulesFile, propertiesFile, exprs, initialState, maxPathLength, simMethod, usp);
			...
			long maxPathLength, SimulationMethod simMethod, UnboundedSimulationParameters usp) throws PrismException
	{
		...
		if (usp != null && usp.getUnboudedSimulationMethod() == UnboundedSimulationMethod.CANDIDATE)
			createNewPath(modulesFile);
		else
		...
		for (int i = 0; i < exprs.size(); i++) {
			try {
				...
				indices[i] = addProperty(exprs.get(i), propertiesFile, usp);
				validPropsCount++;
		...
		if (results.length == 1) {
			...
			if (verbose)
				mainLog.println(propertySamplers.get(indices[0]));
			...
			InterruptedException
	{
		...
		for (int i = 0; i < n; i++) {
			...
			try {
				...
				indices[i] = addProperty(expr, propertiesFile, usp);
				validPropsCount++;
				// Attach a SimulationMethod object to each property's sampler
				SimulationMethod simMethodNew = simMethod.clone();
				propertySamplers.get(indices[i]).setSimulationMethod(simMethodNew);
