...
public class Buchi {
	...
	private List<Generalized.GState> g_init;
	private IntList _final;
	...
	{				/* computes the 'final' value */
		if ((fin != accept) && set.get(_final.getInt(fin)))
	...
	public NBA toNBA(APSet apset) throws PrismException
	{
		...
		Object2IntMap<LTL2BAState> map = new Object2IntOpenHashMap<>();
		int stateindex;
		...
		for (s = bstates.prv; s != bstates; s = s.prv) {
			...
			for (t = s.trans.nxt; t != s.trans; t = t.nxt) {
				...
				nba.nba_i_addEdge(map.getInt(new LTL2BAState(s.id, s._final)), transMonom, map.getInt(new LTL2BAState(t.to.id, t.to._final)));
				for (t1 = t; t1.nxt != s.trans;) {
		...
		if (accept_all)
			nba.nba_i_addEdge(map.getInt(new LTL2BAState(0, accept)), new APMonom(true), map.getInt(new LTL2BAState(0, accept)));
