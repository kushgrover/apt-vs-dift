...
public class Generalized {
	...
	public List<GState> g_init;
	public IntList _final;
	...
	private void make_gtrans(Alternating a, GState s)
	{
		...
		for (i = 0; i < list.size(); i++) {
			...
			p.astate = list.getInt(i);
			p.trans = a.transition.get(list.getInt(i));
			if (p.trans == null)
				trans_exist = 0;
		...
		while (trans_exist != 0) {	/* calculates all the transitions */
			...
			if (t1 != null) {	/* solves the current transition */
				...
				for (i = 0; i < _final.size(); i++)
					if (isFinal(a, s.nodes_set, t1, _final.getInt(i)))
						fin.set(_final.getInt(i));
	...
	private void simplify_gscc(Alternating a)
	{
		...
		scc_final = new ArrayList<MyBitSet>();
		for (i = 0; i < scc_id; i++) {
