...
public class BackwardsReach extends PrismComponent
{
	 ...
	 */
	public double computeProbabilisticReachability(PTA pta, NatBitSet targetLocs, boolean min) throws PrismException
	{
		if (min)
	 ...
	 */
	public BackwardsReachabilityGraph buildBackwardsGraph(PTA pta, NatBitSet targetLocs) throws PrismException
	{
		LocZone lz;
		...
		// Add target states
		count = 0;
		IntIterator iterator = targetLocs.iterator();
		while (iterator.hasNext()) {
			int loc = iterator.nextInt();
								...
								if (src2 != src && lzTmp.loc == lzSrc.loc) {
									...
									if (!zTmp.isEmpty()) {
										...
										for (int iTrans2 = 0; iTrans2 < numTrans2; iTrans2++) {
											List<IntList> edges2 = graph.getList(src2).get(iTrans2);
		...
		mainLog.println(" (" + graph.getInitialStates().size() + " initial, " + graph.getTarget().size() + " target)");
