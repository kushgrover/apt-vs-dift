...
public class DigitalClocks
{
		...
		mf = (ModulesFile) mf.accept(new ASTTraverseModify()
		{
			public Object visit(Module e) throws PrismLangException
			{
				...
				for (String x : cci.getClocksForModule(e.getName())) {
					...
					// Add to update
					up.add(new ExpressionIdent(x), expr);
				}
				ups = new Updates();
	...
	class ComputeClockInformation extends ASTTraverse
	{
		...
		private Object2IntMap<String> clockMaxs;
		private IntSet allClockVals;
		private int scaleFactor;

		public ComputeClockInformation()
		{
			clockLists = new HashMap<>();
			clockMaxs = new Object2IntOpenHashMap<>();
			allClockVals = new IntOpenHashSet();
		}

		private void updateMax(String clock, int val)
		{
			int i = clockMaxs.getOrDefault(clock, -1);
			if (i == -1 || val > i)
		...
		{
			return clockMaxs.getOrDefault(clock, -1);
		}
		...
		{
			int i = clockMaxs.getOrDefault(clock, -1);
			return (i == -1) ? -1 : doScaling ? i / scaleFactor : i;
		}
		 ...
		 */
		private int computeGCD(IntIterable ints)
		{
			int gcd = 0;
			IntIterator iterator = ints.iterator();
			while (iterator.hasNext()) {
				gcd = computeGCD(gcd, iterator.nextInt());
			}
			if (gcd == 0) {
