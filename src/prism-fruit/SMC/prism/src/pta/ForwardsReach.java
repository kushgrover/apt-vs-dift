...
public class ForwardsReach
{
	...
	protected NatBitSet target; // Bit set specifying target states
	protected IntList initialStates; // Initial states
	 ...
	 */
	public ReachabilityGraph buildForwardsGraph(PTA pta, NatBitSet targetLocs, Constraint targetConstraint)
	 ...
	 */
	private ReachabilityGraph buildForwardsGraphFormats10(PTA pta, NatBitSet targetLocs, Constraint targetConstraint)
	throws PrismException
	{
		LocZone init, lz, lz2;
		...
		while (!X.isEmpty()) {
			...
			if (targetLocs.contains(lz.loc) && (targetConstraint == null || lz.zone.isSatisfied(targetConstraint))) {
				target.set(src);
			...
			if (!canDiverge && pta.getTransitions(lz.loc).isEmpty()) {
			...
			if (!canDiverge && graph.trans.get(src).isEmpty()) {
		...
		mainLog.println(" (" + initialStates.size() + " initial, " + target.size() + " target)");
	...
	private ReachabilityGraph buildForwardsGraphFormats09(PTA pta, NatBitSet targetLocs, Constraint targetConstraint)
	throws PrismException
	{
		Zone z;
		LocZone init, lz, lz2;
		...
		mainLog.println("Graph constructed in " + (timer / 1000.0) + " secs.");
		mainLog.print("Graph: " + graph.states.size() + " symbolic states");
		mainLog.println("), " + target.size() + " target states");
