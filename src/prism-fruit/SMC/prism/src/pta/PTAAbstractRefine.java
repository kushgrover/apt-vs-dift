...
public class PTAAbstractRefine extends QuantAbstractRefine
{
	...
	protected NatBitSet targetLocs;
	 ...
	 */
	public double forwardsReachAbstractRefine(PTA pta, NatBitSet targetLocs, Constraint targetConstraint, boolean min)
	...
	protected void initialise() throws PrismException
	{
		...
		// Set initial states
		initialStates.forEach((IntConsumer) abstraction::addInitialState);
	...
	@Override
	protected void rebuildAbstraction(IntSet rebuildStates) throws PrismException
	{
		IntIterator iterator = rebuildStates.iterator();
		while (iterator.hasNext()) {
			int src = iterator.nextInt();
	...
	protected void buildSTPGState(int src) throws PrismException
	{
		...
		buildSTPGStateRec(src, new DBMList(lz.zone), NatBitSets.boundedSet(numValids), valids, map, 0, numValids);
	 ...
	 */
	protected void buildSTPGStateRec(int src, NCZone valid, NatBitSet bitSet, ArrayList<NCZone> valids, int[] map,
			int level, int numValids) throws PrismException
	{
		...
		Distribution distr;
		NatBitSet actionNatBitSet = null;
		int count, i, dest;
		...
		if (level == numValids) {
			...
			if (!valid.isEmpty()) {
				...
				// If using NatBitSets for action labels (as opposed to storing the zones directly)
				...
				if (!storeValidZones)
					actionNatBitSet = NatBitSets.boundedSet(sts.size());
				i = 0;
				for (SymbolicTransition st : sts) {
					...
					if (bitSet.contains(map[i])) {
						...
						if (!distr.isEmpty())
							distrSet.add(distr);
						// If using NatBitSets for action labels, modify the bit set
						// (setting kth bit to denote that kth transition is included)
						if (!storeValidZones)
							actionNatBitSet.set(i);
					}
					i++;
				}
				// Set the (NatBitSet or Zone) action label for the distribution set and add it to the game
				if (storeValidZones)
					distrSet.setAction(valid);
				else
					distrSet.setAction(actionNatBitSet);
	...
	@Override
	protected int splitState(int splitState, List<IntList> choiceLists, IntSet rebuiltStates,
			IntSet rebuildStates) throws PrismException
	{
		...
		choiceCount = 0;
		for (IntList choiceList : choiceLists) {
			choiceCount += choiceList.size();
			z = new DBMList(pta);
			IntIterator iterator = choiceList.iterator();
			while (iterator.hasNext()) {
				int in = iterator.nextInt();
				// If using NatBitSets for action labels (i.e. have not cached zones for reuse)
				// need to build the corresponding zone
				action = ((STPGAbstrSimple) abstraction).getChoice(splitState, in).action;
				if (!storeValidZones) {
					actionNatBitSet = (NatBitSet) action;
					...
					for (i = 0; i < n; i++) {
						...
						(actionNatBitSet.contains(i) ? valids : validCmpls).add((DBMList) st.valid);
					}
	...
	private Zone valid2new(LocZone lz, Transition tr, int[] dests)
	{
		...
		for (Edge edge : tr.getEdges()) {
			...
			for (Int2IntMap.Entry e : edge.getResets()) {
				z2.backReset(e.getIntKey(), e.getIntValue());
			}
			// Intersect
			z.intersect(z2);
			count++;
	...
	{
		return targetLocs.contains(lz.loc) && (targetConstraint == null || lz.zone.isSatisfied(targetConstraint));
	}
}
