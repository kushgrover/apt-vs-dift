...
public class PTAModelChecker extends PrismComponent
{
	...
	private Result checkExpressionProb(ExpressionProb expr) throws PrismException
	{
		...
		mainLog.println("Target (" + exprTarget + ") satisfied by " + targetLocs.size() + " locations.");
	 ...
	 */
	private NatBitSet buildTimeBoundIntoPta(PTA pta, NatBitSet targetLocs, int timeBound, boolean timeBoundStrict)
	{
		...
		for (i = 0; i < numLocs; i++) {
			...
			for (Transition tr : pta.getTransitions(i)) {
				...
				for (Edge e : tr.getEdges()) {
					if (targetLocs.contains(e.getDestination())) {
						toTarget = true;
						break;
					}
				}
		...
		targetLocsNew = NatBitSets.boundedSet(pta.getNumLocations());
		targetLocsNew.set(newTargetLoc);
		if (targetLocs.contains(0)) {
			targetLocsNew.set(0);
		}
	 ...
	 */
	private double computeProbabilisticReachability(NatBitSet targetLocs, boolean min) throws PrismException
	{
	 ...
	 */
	private NatBitSet checkLocationExpression(Expression expr) throws PrismException
	{
		int i, n;
		NatBitSet res;
