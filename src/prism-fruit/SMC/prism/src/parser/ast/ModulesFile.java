...
public class ModulesFile extends ASTElement implements ModelInfo
{
	...
	private List<String> formulaIdents;
	private List<String> constantIdents;
	private List<String> varIdents; // TODO: don't need?
	...
	public ModulesFile()
	{
		...
		globals = new ArrayList<Declaration>();
		...
		initStates = null;
		formulaIdents = new ArrayList<String>();
		constantIdents = new ArrayList<String>();
		varIdents = new ArrayList<String>();
		varDecls = new ArrayList<Declaration>();
	...
	private void checkLabelIdents() throws PrismLangException
	{
		int i, n;
		String s;
		List<String> labelIdents;

		// go thru labels
		n = labelList.size();
		labelIdents = new ArrayList<String>();
		for (i = 0; i < n; i++) {
	...
	public String toString()
	{
		...
		s.append(modelType.toString().toLowerCase()).append("\n\n");
		...
		n = getNumGlobals();
		for (i = 0; i < n; i++) {
			s.append("global ").append(getGlobal(i)).append(";\n");
		}
		if (n > 0) {
			s.append("\n");
		}
		...
		for (i = 0; i < modules.size() - 1; i++) {
			s.append(modules.get(i)).append("\n\n");
		}
		s.append(modules.get(modules.size() - 1)).append("\n");

		for (i = 0; i < systemDefns.size(); i++) {
			s.append("\nsystem ");
			if (systemDefnNames.get(i) != null) {
				s.append("\"").append(systemDefnNames.get(i)).append("\" ");
			}
			s.append(systemDefns.get(i)).append(" endsystem\n");
		}

		n = getNumRewardStructs();
		for (i = 0; i < n; i++) {
			s.append("\n").append(getRewardStruct(i));
		}

		if (initStates != null) {
			s.append("\n").append("init ").append(initStates).append(" endinit\n");
		}
	...
	public ASTElement deepCopy()
	{
		...
		ret.formulaIdents = (formulaIdents == null) ? null : new ArrayList<>(formulaIdents);
		ret.constantIdents = (constantIdents == null) ? null : new ArrayList<>(constantIdents);
		ret.varIdents = (varIdents == null) ? null :  new ArrayList<>(varIdents);
