...
public class CTMCModelChecker extends ProbModelChecker
{
	...
	@Override
	protected StateValues checkRewardCoSafeLTL(Model model, Rewards modelRewards, Expression expr, MinMax minMax, NatBitSet statesOfInterest)
	...
	protected StateValues checkProbBoundedUntil(Model model, ExpressionTemporal expr, MinMax minMax, NatBitSet statesOfInterest) throws PrismException
	{
		...
		b1 = checkExpression(model, expr.getOperand1(), null).getNatBitSet();
		b2 = checkExpression(model, expr.getOperand2(), null).getNatBitSet();
		...
		if (lTime == 0 && uTime == 0) {
			...
			probs = StateValues.createFromNatBitSetAsDoubles(b2, model);
		} else {
	...
	public StateValues doTransient(CTMC ctmc, double t, StateValues initDist) throws PrismException
	{
		...
		if (initDist == null) {
			...
			IntIterator iterator = ctmc.getInitialStates().iterator();
			while (iterator.hasNext()) {
				initDistNew.setDoubleValue(iterator.nextInt(), initVal);
			}
		} else {
	 ...
	 */
	public ModelCheckerResult computeNextProbs(CTMC ctmc, NatBitSet target) throws PrismException
	{
	 ...
	 * @param target Target states
	 */
	public ModelCheckerResult computeReachProbs(CTMC ctmc, NatBitSet target) throws PrismException
	{
		// Construct embedded DTMC and do computation on that
		mainLog.println("Building embedded DTMC...");
		DTMC dtmcEmb = ctmc.getImplicitEmbeddedDTMC();
	 ...
	 * @param target Target states
	 */
	public ModelCheckerResult computeUntilProbs(CTMC ctmc, NatBitSet remain, NatBitSet target) throws PrismException
	{
		// Construct embedded DTMC and do computation on that
		mainLog.println("Building embedded DTMC...");
		DTMC dtmcEmb = ctmc.getImplicitEmbeddedDTMC();
	 ...
	 */
	public ModelCheckerResult computeReachProbs(CTMC ctmc, NatBitSet remain, NatBitSet target, double init[], NatBitSet known) throws PrismException
	{
		// Construct embedded DTMC and do computation on that
		mainLog.println("Building embedded DTMC...");
		DTMC dtmcEmb = ctmc.getImplicitEmbeddedDTMC();
	 ...
	 * @param t      Time bound
	 */
	public ModelCheckerResult computeTimeBoundedReachProbs(CTMC ctmc, NatBitSet target, double t) throws PrismException
	{
	 ...
	 * @param t      Time bound
	 */
	public ModelCheckerResult computeTimeBoundedUntilProbs(CTMC ctmc, NatBitSet remain, NatBitSet target, double t) throws PrismException
	{
		NatBitSet nonAbs = null;
		if (remain != null) {
			nonAbs = (NatBitSet) remain.clone();
	 ...
	 */
	public ModelCheckerResult computeTransientBackwardsProbs(CTMC ctmc, NatBitSet target, NatBitSet nonAbs, double t, double multProbs[]) throws PrismException
	{
		ModelCheckerResult res = null;
		int i, n, iters;
		...
		if (((nonAbs != null && nonAbs.isEmpty()) || (t == 0)) && multProbs == null) {
			...
			res.soln = Utils.intSetToDoubleArray(target, ctmc.getNumStates());
			return res;
		}
		...
		if (multProbs != null) {
			for (i = 0; i < n; i++)
				soln[i] = soln2[i] = target.contains(i) ? multProbs[i] : 0.0;
		} else {
			for (i = 0; i < n; i++)
	...
	public ModelCheckerResult computeCumulativeRewards(CTMC ctmc, MCRewards mcRewards, double t) throws PrismException
	{
		...
		while (iters <= right) {
			// Matrix-vector multiply
			dtmcUnif.mvMult(soln, soln2, null);
			// Swap vectors for next iter
			tmpsoln = soln;
			soln = soln2;
			soln2 = tmpsoln;
	 ...
	 * @param target    Target states
	 */
	public ModelCheckerResult computeReachRewards(CTMC ctmc, MCRewards mcRewards, NatBitSet target) throws PrismException
	{
		int i, n;
		// Build embedded DTMC
		mainLog.println("Building embedded DTMC...");
	...
	@Override public NatBitSet computeExistsGlobally(Model model, NatBitSet A) throws PrismException
	{
		// Construct embedded DTMC and do CTL computation on that
		mainLog.println("Building embedded DTMC...");
		DTMC dtmcEmb = ((CTMC) model).getImplicitEmbeddedDTMC();
	...
	@Override public NatBitSet computeExistsRelease(Model model, NatBitSet A, NatBitSet B) throws PrismException
	{
		// Construct embedded DTMC and do CTL computation on that
		mainLog.println("Building embedded DTMC...");
		DTMC dtmcEmb = ((CTMC) model).getImplicitEmbeddedDTMC();
