...
public class STPGRewardsSimple extends MDPRewardsSimple implements STPGRewards
{
	...
	protected List<List<DoubleList>> nestedTransRewards;
	...
	public STPGRewards liftFromModel(Product<? extends Model> product)
	{
		...
		if (nestedTransRewards != null) {
			for (int s = 0; s < numStatesProd; s++) {
				List<DoubleList> list1 = nestedTransRewards.get(product.getModelState(s));
				if (list1 != null) {
					int n1 = list1.size();
					for (int i = 0; i < n1; i++) {
						DoubleList list2 = list1.get(i);
						int n2 = list2.size();
						for (int j = 0; j < n2; j++) {
							rewardsProd.setNestedTransitionReward(s, i, j, list2.getDouble(j));
						}
					}
				}
			}
