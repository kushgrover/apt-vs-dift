...
public final class FoxGlynn
{
	...
	public int getLeftTruncationPoint()
	{
		return left;
	}

	public int getRightTruncationPoint()
	{
		return right;
	}
	...
	private void run() throws PrismException
	{
		if (q_tmax == 0.0) {
			...
			DoubleList w = new DoubleArrayList(); //stores weights computed so far.
			...
			//run FINDER to get left, right and weight[m]
			{
				...
				for (k = lower_k_1; k <= upper_k_1; k = k + (k == lower_k_1 ? 4 : 1)) {
