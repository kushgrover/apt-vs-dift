...
public abstract class ModelExplicit implements Model
{
	...
	protected IntList initialStates;
	 ...
	 * represented as a String->NatBitSet mapping from their names to the states that satisfy them. */
	protected Map<String, NatBitSet> labels = new TreeMap<>();
	...
	public void copyFrom(ModelExplicit model)
	{
		...
		model.initialStates.forEach((IntConsumer) this::addInitialState);
		model.deadlocks.forEach((IntConsumer) this::addDeadlockState);
	...
	{
		numStates = model.numStates;
		model.initialStates.forEach((IntConsumer) in -> addInitialState(permut[in]));
		model.deadlocks.forEach((IntConsumer) in -> addDeadlockState(permut[in]));
	 ...
	 */
	public void addLabel(String name, NatBitSet states)
	{
	 ...
	 * @param labelStates the NatBitSet with the state set for the label
	 ...
	 */
	public String addUniqueLabel(String prefix, NatBitSet labelStates, Set<String> definedLabelNames)
	{
		String label;
		int i = 0;
	...
	{
		return initialStates.isEmpty() ? -1 : initialStates.iterator().nextInt();
	}
	...
	{
		return StateValues.createFromIntSet(deadlocks.clone(), this);
	}

	@Override
	public int getFirstDeadlockState()
	{
		return deadlocks.firstInt();
	}
