...
public class CTMDPSimple extends MDPSimple implements CTMDP
{
	...
	public boolean isLocallyUniform()
	{
		...
		double d;
		for (i = 0; i < numStates; i++) {
			n = trans.get(i).length;
			if (n < 2)
				continue;
			d = trans.get(i)[0].sum();
			for (j = 1; j < n; j++) {
				if (!PrismUtils.doublesAreCloseAbs(trans.get(i)[j].sum(), d, 1e-12))
					return false;
			}
		}
		return true;
	...
	public MDPSimple buildDiscretisedMDP(double tau)
	{
		...
		getInitialStates().forEach((IntConsumer) mdp::addInitialState);
		for (i = 0; i < numStates; i++) {
			for (Distribution distr : trans.get(i)) {
				...
				for (Int2DoubleMap.Entry e : distr) {
					distrNew.add(e.getIntKey(), (1 - d) * (e.getDoubleValue() / sum));
				}
				distrNew.add(i, d);
