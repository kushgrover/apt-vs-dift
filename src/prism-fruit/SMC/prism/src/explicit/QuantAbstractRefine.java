...
public abstract class QuantAbstractRefine extends PrismComponent
{
	...
	/** NatBitSet of (abstract) target states for property to drive refinement */
	protected NatBitSet target;
	...
	protected NatBitSet known;
	protected IntList refineStates;
	 ...
	 * Notes:
	 * <ul>
	 * <li> The union of all the sets in {@code choiceLists} may not cover all choices in the state to be split.
	 ...
	 */
	protected abstract int splitState(int splitState, List<IntList> choiceLists, IntSet rebuiltStates,
			IntSet rebuildStates) throws PrismException;

	/**
	 * Rebuild the abstraction after a refinement.
	 * @param rebuildStates States that need rebuilding.
	 */
	protected abstract void rebuildAbstraction(IntSet rebuildStates) throws PrismException;
	...
	public double abstractRefine(boolean min) throws PrismException
	{
		...
		lbSoln = Utils.intSetToDoubleArray(target, n);
		ubSoln = new double[n];
		for (i = 0; i < n; i++)
			ubSoln[i] = 1.0;
		known = (NatBitSet) target.clone();
	...
	protected int cheapCheckRefine() throws PrismException
	{
		...
		count = 0;
		i = -1;
		IntIterator iterator = known == null ? IntIterators.fromTo(0, n) : NatBitSets.complementIterator(known, n);
		while (iterator.hasNext()) {
			i = iterator.nextInt();

			//log.println("YY "+i+" "+((STPG)abstraction).steps.get(i));
			if (abstraction.allSuccessorsInSet(i, known)) {
				...
				//
					...
					IntSet rebuildStates = new IntOpenHashSet();
					timer = System.currentTimeMillis();
	...
	protected void modelCheckAbstraction(boolean min) throws PrismException
	{
		...
		IntIterator iterator = abstraction.getInitialStates().iterator();
		while (iterator.hasNext()) {
		...
		// Display results
		mainLog.println(known.size() + "/" + n + " states converged.");
	...
	{
		IntList refinableStates;
		double maxDiff, bound;
		int i, numStates;

		// Empty refinement states lists
		refinableStates = new IntArrayList();
		refineStates = new IntArrayList();
		...
		case FIRST:
		case FIRST_MAX:
			refineStates.add(refinableStates.getInt(0));
			break;
		case LAST:
		case LAST_MAX:
			refineStates.add(refinableStates.getInt(refinableStates.size() - 1));
			break;
		default:
	 ...
	 * @param refineStates States to refine.
	 */
	protected void refine(IntList refineStates) throws PrismException
	{
		IntSet rebuiltStates, rebuildStates;
		...
		rebuiltStates = new IntLinkedOpenHashSet();
		rebuildStates = new IntLinkedOpenHashSet();
		...
		for (i = n - 1; i >= 0; i--) {
			...
			refineState = refineStates.getInt(i);
	 ...
	 */
	protected int refineState(int refineState, IntSet rebuiltStates, IntSet rebuildStates)
			throws PrismException
	{
		List<IntList> choiceLists;
		IntList lbStrat = null, ubStrat = null;
		...
		if (sanityChecks) {
			...
			if (target.contains(refineState)) {
		...
		case VAL:
			...
			case 3:
				...
				IntList newChoiceList;
				newChoiceList = new IntArrayList();
				newChoiceList.add(lbStrat.getInt(0));
				choiceLists.add(newChoiceList);
				newChoiceList = new IntArrayList();
				newChoiceList.add(ubStrat.getInt(0));
				choiceLists.add(newChoiceList);
				break;
			}
			if (verbosity >= 1)
		...
		// Note: we don't have to update 'known' since implicit new elements of a NatBitSet are assumed false
	 ...
	 */
	public void addRemainderIntoChoiceLists(int splitState, List<IntList> choiceLists)
	{
		int nChoices;
		NatBitSet included;
		IntList otherChoices;
		int i;

		nChoices = abstraction.getNumChoices(splitState);
		included = NatBitSets.boundedSet(nChoices);
		choiceLists.forEach(included::addAll);
		otherChoices = new IntArrayList();
		NatBitSets.complementIterator(included, nChoices).forEachRemaining((IntConsumer) otherChoices::add);
	 ...
	 */
	private static void exportToDotFile(String filename, Model abstraction, NatBitSet known, double lbSoln[],
				...
				for (DistributionSet distrs : stpg.getChoices(i)) {
					...
					for (Distribution distr : distrs) {
						...
						for (Int2DoubleMap.Entry e : distr) {
							out.write(nijk + " -> " + e.getIntKey() + " [ label=\"" + e.getDoubleValue() + "\" ];\n");
						}
					}
				}
			}
