...
public class Bisimulation extends PrismComponent
{
	 ...
	 */
	public Model minimise(Model model, List<String> propNames, List<NatBitSet> propBSs) throws PrismException
	{
	 ...
	 * @param propBSs Propositions (satisfying sets of states) to be preserved by bisimulation.
	 */
	private DTMC minimiseDTMC(DTMC dtmc, List<String> propNames, List<NatBitSet> propBSs)
	{
		...
		for (int i = 0; i < numBlocks; i++) {
			for (Int2DoubleMap.Entry e : mdp.getChoice(i, 0)) {
				dtmcNew.setProbability((Integer) mdp.getAction(i, 0), e.getIntKey(), e.getDoubleValue());
			}
		}
	 ...
	 * @param propBSs Propositions (satisfying sets of states) to be preserved by bisimulation.
	 */
	private CTMC minimiseCTMC(CTMC ctmc, List<String> propNames, List<NatBitSet> propBSs)
	{
		// Create initial partition based on propositions
		...
		for (int i = 0; i < numBlocks; i++) {
			for (Int2DoubleMap.Entry e : mdp.getChoice(i, 0)) {
				ctmcNew.setProbability((Integer) mdp.getAction(i, 0), e.getIntKey(), e.getDoubleValue());
			}
		}
	 ...
	 */
	private void initialisePartitionInfo(Model model, List<NatBitSet> propBSs)
	{
		NatBitSet bs1, bs0;
		...
		List<NatBitSet> all = new ArrayList<>();
		bs1 = propBSs.get(0).clone();
		bs0 = bs1.clone();
		...
		for (int j = 0; j < numBlocks; j++) {
			NatBitSet bs = all.get(j);
			IntIterator iterator = bs.iterator();
			while (iterator.hasNext()) {
				partition[iterator.nextInt()] = j;
			}
		}
	}
	...
	private boolean splitDTMC(DTMC dtmc)
	{
		...
		for (s = 0; s < numStates; s++) {
			...
			Iterator<Int2DoubleMap.Entry> iter = dtmc.getTransitionsIterator(s);
			distrNew = new Distribution();
			while (iter.hasNext()) {
				Int2DoubleMap.Entry e = iter.next();
				distrNew.add(partition[e.getIntKey()], e.getDoubleValue());
			}
	 ...
	 * to the minimised model, in the form of labels (stored as NatBitSets).
	 ...
	 */
	private void attachStatesAndLabels(Model model, ModelExplicit modelNew, List<String> propNames, List<NatBitSet> propBSs)
	{
		// Attach states
		if (model.getStatesList() != null) {
		...
		for (int i = 0; i < numProps; i++) {
			...
			NatBitSet propBS = propBSs.get(i);
			NatBitSet propBSnew = NatBitSets.set();
			propBS.forEach((IntConsumer) j -> propBSnew.set(partition[j]));
