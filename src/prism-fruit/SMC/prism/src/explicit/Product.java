...
public abstract class Product<M extends Model> implements ModelTransformation<M, M>
{
	...
	/**
	 * Lifts a NatBitSet over states in the automaton to a NatBitSet
	 ...
	 * @param automataStates a NatBitSet over states of the automaton for this product.
	 */
	public NatBitSet liftFromAutomaton(IntSet automataStates)
	{
		NatBitSet result = NatBitSets.boundedSet(productModel.getNumStates());

		for (int productState = 0; productState < productModel.getNumStates(); productState++) {
			if (automataStates.contains(getAutomatonState(productState))) {
	...
	/**
	 * Lifts a NatBitSet over states in the original model to a NatBitSet
	 * over states in the product model: A bit is set in the
	 * result for a product state if the bit is set in the parameter
	 * for the corresponding original model state.
	 * @param modelStates a NatBitSet over states of the original model for this product.
	 */
	public NatBitSet liftFromModel(IntSet modelStates)
	{
		NatBitSet result = NatBitSets.boundedSet(productModel.getNumStates());

		for (int productState = 0; productState < productModel.getNumStates(); productState++) {
			if (modelStates.contains(getModelState(productState))) {
				result.set(productState);
			}
		}
	...
	public StateValues projectToOriginalModel(StateValues sv) throws PrismException
	{
		...
		IntIterator iterator = productModel.getInitialStates().iterator();
		while (iterator.hasNext()) {
			int productState = iterator.nextInt();
