...
public class NonProbModelChecker extends StateModelChecker
{
	 ...
	 */
	protected StateValues checkExpressionExists(Model model, Expression expr, NatBitSet statesOfInterest) throws PrismException
	{
	 ...
	 */
	protected StateValues checkExpressionForAll(Model model, Expression expr, NatBitSet statesOfInterest) throws PrismException
	{
	 ...
	 */
	protected StateValues checkExistsNext(Model model, Expression expr, NatBitSet statesOfInterest) throws PrismException
	{
		NatBitSet target = checkExpression(model, expr, null).getNatBitSet();
		NatBitSet result = computeExistsNext(model, target, statesOfInterest);

		return StateValues.createFromIntSet(result, model);
	}

	/**
	 * Compute the set of states satisfying E[ X "target" ].
	 * @param model the model
	 * @param target the NatBitSet of states for target
	 * @param statesOfInterest the states of interest ({@code null} = all states)
	 * @return a boolean StateValues, with {@code true} for all states satisfying E[ X "target" ]
	 */
	public NatBitSet computeExistsNext(Model model, NatBitSet target, NatBitSet statesOfInterest) throws PrismException
	{
		NatBitSet result = NatBitSets.set();
		if (statesOfInterest == null) {
			statesOfInterest = NatBitSets.fullSet(model.getNumStates());
		}
		statesOfInterest.forEach((IntConsumer) s -> {
	 ...
	 */
	protected StateValues checkForAllNext(Model model, Expression expr, NatBitSet statesOfInterest) throws PrismException
	{
		NatBitSet target = checkExpression(model, expr, null).getNatBitSet();
		NatBitSet result = computeForAllNext(model, target, statesOfInterest);
	 ...
	 * @param model the model
	 * @param target the NatBitSet of states in target
	 * @param statesOfInterest the states of interest ({@code null} = all states)
	 * @return a boolean StateValues, with {@code true} for all states satisfying A[ X "target" ]
	 */
	public NatBitSet computeForAllNext(Model model, NatBitSet target, NatBitSet statesOfInterest) throws PrismException
	{
		NatBitSet result = NatBitSets.set();
		if (statesOfInterest == null) {
			statesOfInterest = NatBitSets.fullSet(model.getNumStates());
	...
	protected StateValues checkExistsUntil(Model model, Expression exprA, Expression exprB) throws PrismException {
		...
		NatBitSet A = checkExpression(model, exprA, null).getNatBitSet();
		// the set of states satisfying exprB
		NatBitSet B = checkExpression(model, exprB, null).getNatBitSet();

		NatBitSet result = computeExistsUntil(model, A, B);
	 ...
	 * Compute the set of states satisfying E[ "a" U "b" ].
	 * @param model the model
	 * @param A the NatBitSet of states for "a"
	 * @param B the NatBitSet of states for "b"
	 * @return a boolean StateValues, with {@code true} for all states satisfying E[ "a" U "b" ]
	 */
	public NatBitSet computeExistsUntil(Model model, NatBitSet A, NatBitSet B) throws PrismException
	{
 		PredecessorRelation pre = model.getPredecessorRelation(this, true);
 		return PredecessorRelations.calculatePreStar(pre, A, B, B);
	}
	 ...
	 * @return a boolean StateValues, with {@code true} for all states satisfying E[ G "a" ]
	 */
	public NatBitSet computeExistsGlobally(Model model, NatBitSet A) throws PrismException
	{
		return computeExistsRelease(model, NatBitSets.set(), A);
	}
	...
	protected StateValues checkExistsRelease(Model model, Expression exprA, Expression exprB) throws PrismException
	{
		...
		T.forEach((IntConsumer) s -> {
			if (AandB.contains(s)) {
				return;
			}

			int i=0;
			for (IntIterator it = model.getSuccessorsIterator(s); it.hasNext(); it.nextInt()) {
				i++;
			}
			count[s]=i;
		});
		...
		while (!E.isEmpty()) {
			...
			pre.getPre(t).forEach((IntConsumer) s -> {
		...
		return StateValues.createFromIntSet(T, model);
	}
	 ...
	 * exists a path satisfying G "b" or "b" U ("a&b")
	 * @param model the model
	 * @param A the NatBitSet for the states in "a"
	 * @param A the NatBitSet for the states in "a"
	 * @return a boolean StateValues, with {@code true} for all states satisfying E[ "a" R "b" ]
	 */
	public NatBitSet computeExistsRelease(Model model, NatBitSet A, NatBitSet B) throws PrismException
	{
		PredecessorRelation pre = model.getPredecessorRelation(this, true);
		...
		// and for which we have proven that they do not satisfy E[ a R b ]
		// Initially, it contains the complement of 'T'
		NatBitSet E = NatBitSets.boundedFilledSet(model.getNumStates());
		E.andNot(T);
	 ...
	 */
	protected StateValues checkExistsLTL(Model model, Expression expr, NatBitSet statesOfInterest) throws PrismException
	{
		...
		mainLog.println(" " + product.getProductModel().infoString() + ", " + product.getAcceptingStates().size() + " states accepting");
		...
		if (product.getAcceptingStates().isEmpty()) {
			...
			return StateValues.createFromIntSet(NatBitSets.set(), model);
		}
		...
		int accepting = 0;
		int sccs = 0;
		NatBitSet acceptingSCCs = NatBitSets.set();
		for (NatBitSet scc : sccComputer.getSCCs()) {
			sccs++;
		...
		NatBitSet allStates = NatBitSets.boundedFilledSet(product.getProductModel().getNumStates());
		...
		NatBitSet resultProduct = computeExistsUntil(product.getProductModel(), allStates, acceptingSCCs);
		StateValues svProduct = StateValues.createFromIntSet(resultProduct, product.getProductModel());
