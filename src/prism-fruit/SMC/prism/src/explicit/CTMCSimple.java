...
public class CTMCSimple extends DTMCSimple implements CTMC
{
	...
	@Override
	public double getMaxExitRate(NatBitSet subset)
	{
		int i;
		double d, max = Double.NEGATIVE_INFINITY;
	...
	@Override
	public double getDefaultUniformisationRate(NatBitSet nonAbs)
	{
	...
	public DTMCSimple buildEmbeddedDTMC()
	{
		...
		getInitialStates().forEach((IntConsumer) dtmc::addInitialState);
		for (i = 0; i < numStates; i++) {
			distr = trans.get(i);
			d = distr.sum();
			if (d == 0) {
				...
				for (Int2DoubleMap.Entry e : distr) {
					dtmc.setProbability(i, e.getIntKey(), e.getDoubleValue() / d);
				}
			}
		}
		return dtmc;
	...
	public DTMCSimple buildUniformisedDTMC(double q)
	{
		...
		for (i = 0; i < numStates; i++) {
			...
			distr = trans.get(i);
			for (Int2DoubleMap.Entry e : distr) {
				dtmc.setProbability(i, e.getIntKey(), e.getDoubleValue() / q);
			}
