...
public class BirthProcess
{
	...
	public double calculateNextProb(double rate) throws PrismException
	{
		...
		if ((jumpRates.size() != 1) && (Math.abs(rate - jumpRates.getDouble(jumpRates.size() - 2)) > 1E-100)) {
			if (avoidBirthComputation) {
				recompute = true;
			}
		...
		if (recompute) {
			for (stageNr = 0; stageNr < jumpRates.size(); stageNr++) {
				result = compNextStageProb(jumpRates.getDouble(stageNr));
			}
		} else {
			if (avoidBirthComputation) {
