...
public class ConstructModel extends PrismComponent
{
	...
	public Model constructModel(ModelGenerator modelGen, boolean justReach) throws PrismException
	{
		...
		if (sort) {
			...
			statesList = states.toPermutedList(permut);
		...
		if (!justReach) {
			switch (modelType) {
			case DTMC:
				model = sort ? new DTMCSimple(dtmc, permut) : dtmc;
				break;
			case CTMC:
				model = sort ? new CTMCSimple(ctmc, permut) : ctmc;
				break;
			case MDP:
				if (buildSparse) {
	...
	private void attachLabels(ModelGenerator modelGen, ModelExplicit model) throws PrismException
	{
		...
		NatBitSet bitsets[] = new NatBitSet[numLabels];
		for (int j = 0; j < numLabels; j++) {
			bitsets[j] = NatBitSets.set();
		}
