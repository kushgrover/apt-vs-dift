...
public class ProbModelChecker extends NonProbModelChecker
{
	...
	// Method used to solve DTMCs
	protected DTMCSolnMethod dtmcSolnMethod = DTMCSolnMethod.LINEAR_EQUATIONS;
	...
	{
		POWER, JACOBI, GAUSS_SEIDEL, BACKWARDS_GAUSS_SEIDEL, EXACT_LINEAR_PROGRAMMING, JOR, SOR, BACKWARDS_SOR, DECOMPOSITION;
	...
	{
		VALUE_ITERATION, GAUSS_SEIDEL, POLICY_ITERATION, MODIFIED_POLICY_ITERATION, LINEAR_PROGRAMMING, INTERVAL_ITERATION, MEC_DECOMPOSITION;
	...
	public enum DTMCSolnMethod
	{
		LINEAR_EQUATIONS, INTERVAL_ITERATION;
	...
	{
		VALUE_ITERATION, GAUSS_SEIDEL, POLICY_ITERATION, MODIFIED_POLICY_ITERATION, LINEAR_PROGRAMMING, EXACT_LINEAR_PROGRAMMING, INTERVAL_ITERATION
	}
	...
	public ProbModelChecker(PrismComponent parent) throws PrismException
	{
		...
		if (settings != null) {
			...
			if (s.equals("Value iteration")) {
				...
				setMDPSolnMethod(MDPSolnMethod.INTERVAL_ITERATION);
			} else if (s.equals("MEC decomposition")) {
				setMDPSolnMethod(MDPSolnMethod.MEC_DECOMPOSITION);
			} else {
				throw new PrismNotSupportedException("Explicit engine does not support MDP solution method \"" + s + "\"");
			}
			s = settings.getString(PrismSettings.PRISM_DTMC_SOLN_METHOD);
			if (s.equals("Interval iteration")) {
				setDTMCSolnMethod(DTMCSolnMethod.INTERVAL_ITERATION);
			} else {
				setDTMCSolnMethod(DTMCSolnMethod.LINEAR_EQUATIONS);
			}
			// PRISM_TERM_CRIT
	...
	public void inheritSettings(ProbModelChecker other)
	{
		...
		setDTMCSolnMethod(other.getDTMCSolnMethod());
	 ...
	 */
	@Override public void setVerbosity(int verbosity)
	{
	...
	/**
	 * Set method used to solve DTMCs.
	 */
	public void setDTMCSolnMethod(DTMCSolnMethod dtmcSolnMethod)
	{
		this.dtmcSolnMethod = dtmcSolnMethod;
	}
	...
	@Override public int getVerbosity()
	{
		return verbosity;
	}
	...
	public DTMCSolnMethod getDTMCSolnMethod()
	{
		return dtmcSolnMethod;
	}
	 ...
	 */
	protected StateValues checkExpressionStrategy(Model model, ExpressionStrategy expr, NatBitSet statesOfInterest) throws PrismException
	{
	 ...
	 * Model check a P operator expression and return the values for the statesOfInterest.
 	 * @param statesOfInterest the states of interest, see checkExpression()
	 */
	protected StateValues checkExpressionProb(Model model, ExpressionProb expr, NatBitSet statesOfInterest) throws PrismException
	{
	 ...
	 * @param statesOfInterest the states of interest, see checkExpression()
	 */
	protected StateValues checkExpressionProb(Model model, ExpressionProb expr, boolean forAll, Coalition coalition, NatBitSet statesOfInterest) throws PrismException
	{
		...
		// Otherwise, compare against bound to get set of satisfying states
			...
			NatBitSet sol = probs.getNatBitSetFromInterval(opInfo.getRelOp(), opInfo.getBound());
			probs.clear();
			return StateValues.createFromIntSet(sol, model);
		}
	}
	 ...
	 * @param statesOfInterest the states of interest, see checkExpression()
	 */
	protected StateValues checkProbPathFormula(Model model, Expression expr, MinMax minMax, NatBitSet statesOfInterest) throws PrismException
	{
		// Test whether this is a simple path formula (i.e. PCTL)
		// and whether we want to use the corresponding algorithms
		boolean useSimplePathAlgo = expr.isSimplePathFormula();

		if (useSimplePathAlgo && settings != null &&
	 ...
	 */
	protected StateValues checkProbPathFormulaSimple(Model model, Expression expr, MinMax minMax, NatBitSet statesOfInterest) throws PrismException
	{
	 ...
	 */
	protected StateValues checkProbNext(Model model, ExpressionTemporal expr, MinMax minMax, NatBitSet statesOfInterest) throws PrismException
	{
		// Model check the operand for all states
		NatBitSet target = checkExpression(model, expr.getOperand2(), null).getNatBitSet();
	 ...
	 */
	protected StateValues checkProbBoundedUntil(Model model, ExpressionTemporal expr, MinMax minMax, NatBitSet statesOfInterest) throws PrismException
	{
		...
		NatBitSet remain = checkExpression(model, expr.getOperand1(), null).getNatBitSet();
		NatBitSet target = checkExpression(model, expr.getOperand2(), null).getNatBitSet();
		...
		if (windowSize == null) {
			...
			sv = StateValues.createFromNatBitSetAsDoubles(target, model);
		} else {
	 ...
	 */
	protected StateValues checkProbUntil(Model model, ExpressionTemporal expr, MinMax minMax, NatBitSet statesOfInterest) throws PrismException
	{
		// Model check operands for all states
		NatBitSet remain = checkExpression(model, expr.getOperand1(), null).getNatBitSet();
		NatBitSet target = checkExpression(model, expr.getOperand2(), null).getNatBitSet();
	 ...
	 */
	protected StateValues checkExpressionReward(Model model, ExpressionReward expr, NatBitSet statesOfInterest) throws PrismException
	{
	 ...
	 * Model check an R operator expression and return the values for all states.
	 */
	protected StateValues checkExpressionReward(Model model, ExpressionReward expr, boolean forAll, Coalition coalition, NatBitSet statesOfInterest) throws PrismException
	{
		...
		// Print out rewards
		if (mainLog.isLoggable(PrismLog.VL_ALL)) {
		...
		// Otherwise, compare against bound to get set of satisfying states
			...
			NatBitSet sol = rews.getNatBitSetFromInterval(opInfo.getRelOp(), opInfo.getBound());
			rews.clear();
			return StateValues.createFromIntSet(sol, model);
		}
	}
	 ...
	 */
	protected StateValues checkRewardFormula(Model model, Rewards modelRewards, Expression expr, MinMax minMax, NatBitSet statesOfInterest) throws PrismException
	{
		...
		if (expr.getType() instanceof TypePathDouble) {
			...
			case ExpressionTemporal.R_C:
				...
				break;
			case ExpressionTemporal.R_M:
				rewards = checkRewardAverage(model, modelRewards, exprTemp, minMax);
				break;
			default:
	...
	/**
	 * Compute expected rewards for a average reward operator.
	 */
	protected StateValues checkRewardAverage(Model model, Rewards modelRewards, ExpressionTemporal expr, MinMax minMax) throws PrismException
	{
		// Check that there is no upper time bound
		if (expr.getUpperBound() != null) {
			throw new PrismException("This is not a average reward operator");
		}
		...
		switch (model.getModelType()) {
		case DTMC:
			res = ((DTMCModelChecker) this).computeAverageReward((DTMC) model, (MCRewards) modelRewards);
			break;
		case MDP:
			res = ((MDPModelChecker) this).computeAverageReward((MDP) model, (MDPRewards) modelRewards, minMax.isMin());
			break;
		default:
			throw new PrismNotSupportedException("Explicit engine does not yet handle the " + expr.getOperatorSymbol() + " reward operator for "
	 ...
	 */
	protected StateValues checkRewardPathFormula(Model model, Rewards modelRewards, Expression expr, MinMax minMax, NatBitSet statesOfInterest) throws PrismException
	{
	 ...
	 */
	protected StateValues checkRewardReach(Model model, Rewards modelRewards, ExpressionTemporal expr, MinMax minMax, NatBitSet statesOfInterest) throws PrismException
	{
	 ...
	 */
	protected StateValues checkRewardCoSafeLTL(Model model, Rewards modelRewards, Expression expr, MinMax minMax, NatBitSet statesOfInterest) throws PrismException
	{
		// To be overridden by subclasses
	...
	protected StateValues checkSteadyStateFormula(Model model, Expression expr, MinMax minMax) throws PrismException
	{
		...
		NatBitSet b = checkExpression(model, expr, null).getNatBitSet();

		// Compute/return the probabilities
		ModelCheckerResult res = null;
		switch (model.getModelType()) {
		case DTMC:
			double multProbs[] = Utils.intSetToDoubleArray(b, model.getNumStates());
	...
	public StateValues buildInitialDistribution(Model model) throws PrismException
	{
		...
		IntIterator iterator = model.getInitialStates().iterator();
		while (iterator.hasNext()) {
			int in = iterator.nextInt();
