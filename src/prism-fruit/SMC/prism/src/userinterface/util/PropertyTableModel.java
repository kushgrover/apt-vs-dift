...
public class PropertyTableModel extends AbstractTableModel implements Observer
{
	...
	private IntArrayList groupStarts;
	private IntArrayList groupSizes;
	...
	public PropertyTableModel()
	{
		...
		groupStarts = new IntArrayList();
		groupSizes = new IntArrayList();
		...
		while (it.hasNext()) {
			...
			if (last == null) {
				...
				if (!po.getDescriptor().equals(""))
					ownerList.append("\'").append(po.getDescriptor()).append("\'");
				...
				tempName += " " + ownerList + "";
				ownerList = new StringBuilder();
				groupNames.add(tempName);
				groupSizes.add(currGroupCount);
				currGroupCount = 0;
				currGroupCount++;
				ownerList.append(po.getDescriptor());
				if (!po.getDescriptor().equals(""))
					tempName = po.getClassDescriptor() + " \'" + po.getDescriptor() + "\'";
	...
	public int getRowCount()
	{
		...
		PropertyOwner firstInGroup = owners.get(groupStarts.getInt(currentGroup));
	...
	{
		return groupSizes.getInt(currentGroup);
	}

	protected int getCurrentGroupStart()
	{
		return groupStarts.getInt(currentGroup);
	}
