	...
	public void run()
	{
		...
		if (allAtOnce) {
			...
			try {
				...
				// do simulation
				results = prism.modelCheckSimulatorSimultaneously(pf, properties, definedPFConstants, initialState, maxPathLength, method, null);
				method.reset();
			} catch (Exception e) {
		...
		// do each property individually
			...
			for (int i = 0; i < pf.getNumProperties(); i++) {
				...
				try {
					...
					// do simulation
					result = prism.modelCheckSimulator(pf, pf.getProperty(i), definedPFConstants, initialState, maxPathLength, method, null);
					method.reset();
