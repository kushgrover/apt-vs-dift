...
public class GUISimulator extends GUIPlugin implements MouseListener, ListSelectionListener, PrismSettingsListener
{
	...
	public void newPathAfterParse()
	{
		...
		try {
			...
			PropertiesFile pf;
			try {
				pf = Prism.parsePropertiesString(parsedModel, guiProp.getConstantsString().toString() + guiProp.getLabelsString());
		...
		if (pathActive) {
			...
			for (int i = 0; i < gpl.getNumProperties(); i++) {
				...
				if (allConstantsDefined &&
					...
					// TODO fix the null
					thePathFormulaeList.addProperty(gp.getProperty(), propertiesFile, null);
				}
			}
		}
	}
